...
Role = Literal[
    ...
    "review",    # every N steps, and once at the end: label what a batch of
                 # passing steps did wrong that no single step's own checks
                 # could see (never decides what to do -- same split as diagnose)
    "ask",       # /bytheway:      an isolated one-off question, no task state
    "locate",    # many per task:  rank retrieval's candidate entities by
                 # relevance to a query (LocAgent). Its own identity, not
                 # "plan" -- it used to borrow that label and the trace lied
                 # about what call was actually running.
]

ROLES: tuple[Role, ...] = (
    "classify", "plan", "execute", "diagnose", "review", "ask", "locate")
...
EventKind = Literal[
    ...
    "verify",
    "review",      # L2: an LLM pass over a batch's accumulated diff
    "compact",
    "checkpoint",
    "error",
]
